...
).start()  # выполнить до импорта заменяемой функции
...
test_account = {"email": "kot@pes.com", "password": "1234"}
...
async def register_user(setup_database, async_client):
    response = await async_client.post("/auth/register", json=test_account)


@pytest.fixture(scope="session")
async def authenticated_async_client(register_user, async_client):
    response = await async_client.post("/auth/login", json=test_account)
